...
const skillDescriptions: { [key: string]: { description: string; experience: string } } = {
  'Houdini': {
    description: 'Pracowałem przy wielu projektach przez ostatnie 15 lat, wykorzystując Houdini do tworzenia zaawansowanych efektów wizualnych, symulacji cząsteczek i proceduralnego modelowania.',
    experience: '15+ lat doświadczenia'
  },
  'Nuke': {
    description: 'Współpracowałem z Netflix nad serialem "Liberator", gdzie odpowiadałem za zaawansowane compositing i integrację efektów wizualnych.',
    experience: 'Netflix - "Liberator" TV series'
  },
  'Unreal Engine': {
    description: 'W Platige Image od 4 lat wykorzystuję Unreal Engine do real-time visualizations. Nasza praca została nagrodzona AEAF za 2. miejsce.',
    experience: '4+ lat w Platige Image, nagroda AEAF'
  },
  'SynthEyes': {
    description: 'Uwielbiam tracking 3D! SynthEyes to moje ulubione narzędzie do precyzyjnego śledzenia ruchu kamery i obiektów w profesjonalnych produkcjach VFX.',
    experience: 'Specjalista 3D tracking'
  },
  'After Effects': {
    description: 'Wykorzystuję After Effects do motion graphics, kompozycji i post-produkcji w projektach reklamowych i filmowych.',
    experience: 'Motion graphics i compositing'
  },
  'Python': {
    description: 'Python wykorzystuję głównie do tworzenia i optymalizacji pipeline\'ów produkcyjnych, automatyzacji procesów i tworzenia narzędzi dla artystów.',
    experience: 'Pipeline development i automatyzacja'
  },
  'Blender': {
    description: 'Blender towarzyszył mi przez ostatnie 20 lat mojej kariery. To było moje pierwsze poważne narzędzie 3D, z którym nauczyłem się podstaw modelowania i animacji.',
  ...
  return (
    <div className="mt-24">
      ...
      <div className="max-w-7xl mx-auto">
        {/* Skills Grid - Horizontal on desktop, vertical on mobile */}
              ...
              <div key={index} className="relative">
                ...
                <button
                  ...
                  className={`w-full h-32 flex flex-col items-center justify-center bg-gray-800/50 rounded-lg border border-gray-700 hover:border-[#c770f0] hover:scale-105 hover:shadow-lg hover:shadow-[#c770f0]/20 transition-all duration-300 ${
                    isExpanded ? 'border-[#c770f0] bg-gray-700/30' : ''
                  }`}
                >
        ...
        {/* Dropdown Panel - Appears below the entire grid */}
          ...
          {expandedSkill && (
            <div className="bg-gray-800/50 rounded-lg border border-gray-700 p-8">
              <div className="flex items-center mb-6">
                <div className="text-[#c770f0] mr-4 text-4xl">
                  {SKILLS.find(skill => skill.name === expandedSkill)?.icon}
                </div>
                <div>
                  <h3 className="text-2xl font-bold text-white">{expandedSkill}</h3>
                  <p className="text-[#c770f0] font-semibold">
